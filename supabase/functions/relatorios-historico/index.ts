...
const buildErrorResponse = (message, status = 400)=>new Response(JSON.stringify({
    error: message
  }), {
    status,
    headers: {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: solicitantePerfil, error: perfilError } = await adminClient.from("perfis").select("id").eq("user_id", user.id).single();
    ...
    if (!autorizado) {
      const { data: vinculo, error: vinculoError } = await adminClient.from("vinculos_familiares").select("id_idoso").eq("id_familiar", solicitantePerfil.id).eq("id_idoso", perfil_id).maybeSingle();
      if (vinculoError) {
    ...
    const { data: eventos, error: eventosError } = await adminClient.from("historico_eventos").select("id, data_prevista, status, tipo_evento, evento_id, horario_programado").eq("perfil_id", perfil_id).gte("data_prevista", dataInicioISO).lte("data_prevista", dataFimISO).order("data_prevista", {
      ascending: true
    });
    if (eventosError) {
    ...
    return buildErrorResponse(error instanceof Error ? error.message : "Erro interno ao processar histórico.", 500);
  }
});
...
const percentage = (partial, total)=>total === 0 ? 0 : Number((partial / total * 100).toFixed(2));
const buildAnalyticsResponse = (eventos)=>{
  ...
  const tendenciaArray = Object.entries(tendenciaDiaria).sort(([a], [b])=>a > b ? 1 : -1).map(([data, { total, confirmados }])=>({
      data,
      percentual: percentage(confirmados, total),
      total,
      confirmados
    }));
  const turnosFormatados = Object.entries(turnoStats).reduce((acc, [turno, stats])=>({
      ...acc,
      [turno]: {
        total: stats.total,
  ...
  const resumoPorTipo = Object.entries(tipoStats).reduce((acc, [tipo, stats])=>({
      ...acc,
      [tipo]: {
        total: stats.total,
        confirmados: stats.confirmados,
  ...
  return {
    ...
    kpis: {
      ...
      pontualidade_media_minutos: pontualidadeSamples === 0 ? null : Number((pontualidadeTotalMin / pontualidadeSamples).toFixed(2))
    },
    graficos: {
