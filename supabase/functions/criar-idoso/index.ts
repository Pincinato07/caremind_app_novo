...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: perfilFamiliar, error: perfilError } = await supabaseAdmin.from('perfis').select('id').eq('user_id', familiarUserId).single();
    if (perfilError || !perfilFamiliar) {
      return new Response(JSON.stringify({
        error: 'Perfil do familiar não encontrado'
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    if (!nome_idoso || typeof nome_idoso !== 'string' || nome_idoso.trim().length === 0) {
      return new Response(JSON.stringify({
        error: 'Nome do idoso é obrigatório e não pode estar vazio'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (!email_idoso || typeof email_idoso !== 'string' || email_idoso.trim().length === 0) {
      return new Response(JSON.stringify({
        error: 'Email do idoso é obrigatório e não pode estar vazio'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (createUserError) {
      ...
      if (createUserError.message?.includes('already registered') || createUserError.message?.includes('already exists') || createUserError.message?.includes('User already registered')) {
        ...
        const { data: existingPerfil } = await supabaseAdmin.from('perfis').select('id').eq('user_id', novoUserIdoso).maybeSingle();
        if (existingPerfil) {
        ...
        const { data: newPerfil, error: createPerfilError } = await supabaseAdmin.from('perfis').insert({
          id: perfilIdosoId,
          user_id: novoUserIdoso,
          nome: nomeNormalizado,
          tipo: 'idoso'
        ...
        if (createPerfilError || !newPerfil) {
          ...
          return new Response(JSON.stringify({
            error: `Erro ao criar perfil: ${createPerfilError?.message || 'Erro desconhecido'}`
          }), {
            status: 500,
            headers: {
              ...corsHeaders,
      ...
      return new Response(JSON.stringify({
        error: 'Erro ao criar usuário: usuário não foi retornado'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
      ...
      const { data: existingPerfil } = await supabaseAdmin.from('perfis').select('id, nome, tipo').eq('user_id', novoUserIdoso).maybeSingle();
      if (existingPerfil) {
        ...
        if (!existingPerfil.nome || existingPerfil.nome.trim() === '' || existingPerfil.tipo !== 'idoso') {
          ...
          const { data: updatedPerfil } = await supabaseAdmin.from('perfis').update({
            nome: nomeNormalizado,
            tipo: 'idoso'
          }).eq('id', perfilIdosoId).select().single();
          if (updatedPerfil) {
        ...
        if (createPerfilError) {
          ...
          if (createPerfilError.code === '23505' || createPerfilError.message?.includes('duplicate key')) {
            ...
            const { data: existingPerfil } = await supabaseAdmin.from('perfis').select().eq('user_id', novoUserIdoso).single();
            if (existingPerfil) {
          ...
          await supabaseAdmin.auth.admin.deleteUser(novoUserIdoso);
          return new Response(JSON.stringify({
            error: 'Erro ao criar perfil: perfil não foi retornado'
          }), {
            status: 500,
            headers: {
              ...corsHeaders,
    ...
    const { error: createVinculoError } = await supabaseAdmin.from('vinculos_familiares').insert({
    ...
    if (createVinculoError) {
      ...
      return new Response(JSON.stringify({
        error: `Erro ao criar vínculo: ${createVinculoError?.message || 'Erro desconhecido'}`
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    if (!perfilIdoso || !perfilIdoso.nome || perfilIdoso.nome.trim() === '') {
      ...
      return new Response(JSON.stringify({
        error: 'Erro: perfil foi criado mas o nome não foi salvo corretamente'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
