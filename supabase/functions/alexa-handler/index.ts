  ...
  try {
    ...
    if (requestType === 'IntentRequest') {
      ...
      if (intentName === 'ConfirmarEventoIntent') {
        ...
        const { data: eventoAtualizado, error: updateError } = await supabaseClient.from('historico_eventos').update({
          status: 'concluido',
          horario_confirmacao: new Date().toISOString()
        }).eq('perfil_id', perfilId) // [BUGFIX] Chave correta
        .eq('status', 'pendente').order('data_prevista', {
          ascending: true
        }) // Pega o mais próximo/antigo
        .limit(1).select('titulo, tipo_evento') // Pede o 'titulo' de volta
        .single(); // Espera apenas um
        if (updateError) {
